...
folder='/nobackup/csjone15/pleiades_llc_recipes/python_cli_data_export/surf_extract/surf_wind'
...
urls = ["/nobackup/csjone15/pleiades_llc_recipes/python_cli_data_export/surf_extract/surf_wind/" + p for p in arr]
#urls = ["/nobackup/csjone15/pleiades_llc_recipes/python_cli_data_export/grid/netcdf_files/" + p for p in arr]

def upload_a_file(u):
    fname = u.split('/')[-1]
    s3.Bucket('cnh-bucket-1').upload_file(u, f"llc_wind/netcdf_files/{fname}")
